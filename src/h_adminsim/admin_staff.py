...
class SchedulingAdminStaffAgent:
    ...
    def _init_env(self, **kwargs):
        ...
        Initialize the environment with default settings.
        """
        assert self.target_task in ['first_visit_intake', 'first_visit_scheduling'], \
            log(colorstr("red", f"Unsupported target task: {self.target_task}. Supported tasks are `first_visit_intake` and `first_visit_scheduling`."))
